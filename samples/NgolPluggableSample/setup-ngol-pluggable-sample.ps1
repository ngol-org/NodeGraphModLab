# setup-ngol-pluggable-sample.ps1 — ngol-plugin/ を組み立てる
#
# 展開済みリリースzipの NGOL/ フォルダ（NodeGraphModLab.Core.dll・NodeAPI.dll・HostLogging.dll・
# 依存DLL・WebUI/・Nodes/Builtin/ 等）を元に、NgolPluggableSample.exe がそのまま起動できる
# ngol-plugin/ を組み立てる。
#
...
#
# Usage:
#   .\setup-ngol-pluggable-sample.ps1 -SourceDir "<展開したNGOL/フォルダのパス>" -OutputDir ".\ngol-plugin"
...
Write-Host "`n=== NgolPluggableSample: ngol-plugin 組み立て ===" -ForegroundColor Cyan
...
Write-Host "    NgolPluggableSample.exe `"$OutputDir`"" -ForegroundColor DarkGray
