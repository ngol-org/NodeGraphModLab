...
⚠️ **`connections` を書く前に、登場する全ノードタイプについて `get_node_detail` でポート名を確認すること。**
本ドキュメントは JSON の構造を示すだけで、**個々のノードのポート名は含まない**。

**ポート名を間違えても実行は成功する。** 実在性はエンジン側で検証されず、
値が渡らないまま各ノードは既定値で動いて `[OK]` を返す。
WebUI 上では該当ポートのハンドルが解決できず**線が描かれない**が、
実行結果からは異常を検出できない。

ポート名はノードごとに異なり、推測が当たるとは限らない。実例:

| ノード | 出力 | 入力 |
|---|---|---|
| `ngol.logic.const_number` | `value` | — |
| `ngol.snapshot.number` | `number` | **`number`**（`value` ではない） |
| `ngol.snapshot.string` | `string` | **`string`** |

`ngol.snapshot.*` は**型名がそのままポート名**で、入力・出力とも同名である。
